...
function ScenarioList({ onSelect, onAddScenario, onEditScenario, scenarios, setScenarios }) {
  ...
  useEffect(() => {
    const fetchAndSetScenarios = async () => {
      ...
      try {
        ...
        const scenarioList = await backendService.fetchScenarios();
  ...
  const handleCloneScenario = async (scenarioToClone) => {
    ...
    if (newName && newName.trim()) {
      ...
      // <<< [추가 끝] >>>
        ...
        try {
          const newScenario = await backendService.cloneScenario({
  ...
  const handleDeleteScenario = async (scenarioId) => {
    ...
    if (confirmed) {
      try {
        await backendService.deleteScenario({ scenarioId });
    ...
    <div style={styles.container}>
      ...
      <ul style={styles.list}>
        {sortedScenarios.map(scenario => {
          const lastUsedAtDate = scenario.lastUsedAt ? new Date(scenario.lastUsedAt) : null;
