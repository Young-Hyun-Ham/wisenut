...
const useStore = create((set, get) => ({
  ...
  fetchNodeColors: async () => {
    try {
      const settings = await backendService.fetchNodeColors();
      if (settings && settings.colors) {
        const mergedColors = ALL_NODE_TYPES.reduce((acc, type) => {
          acc[type] = settings.colors[type] || defaultColors[type];
          return acc;
        }, {});
        set({ nodeColors: mergedColors });
      } else {
        await backendService.saveNodeColors(defaultColors);
        set({ nodeColors: defaultColors });
      }
    } catch (error) {
      console.error("Failed to fetch node colors:", error);
      set({ nodeColors: defaultColors });
    }
  },
  ...
  fetchNodeTextColors: async () => {
    try {
      const settings = await backendService.fetchNodeTextColors();
      if (settings && settings.colors) {
        const mergedTextColors = ALL_NODE_TYPES.reduce((acc, type) => {
          acc[type] = settings.colors[type] || defaultTextColors[type];
          return acc;
        }, {});
        set({ nodeTextColors: mergedTextColors });
      } else {
        await backendService.saveNodeTextColors(defaultTextColors);
        set({ nodeTextColors: defaultTextColors });
      }
    } catch (error) {
      console.error("Failed to fetch node text colors:", error);
      set({ nodeTextColors: defaultTextColors });
    }
  },
  ...
  fetchNodeVisibility: async () => {
    try {
      const settings = await backendService.fetchNodeVisibility();
      if (settings && Array.isArray(settings.visibleNodeTypes)) {
        set({ visibleNodeTypes: settings.visibleNodeTypes });
      } else {
        await backendService.saveNodeVisibility(defaultVisibleNodeTypes);
  ...
  setNodeVisibility: async (nodeType, isVisible) => {
    ...
    try {
      await backendService.saveNodeVisibility(newVisibleArray);
    } catch (error) {
  ...
  setNodeColor: async (type, color) => {
    ...
    try {
      await backendService.saveNodeColors(newColors);
    } catch (error) {
  ...
  setNodeTextColor: async (type, color) => {
    ...
    try {
      await backendService.saveNodeTextColors(newTextColors);
    } catch (error) {
  ...
  addScenarioAsGroup: async (scenario, position) => {
    ...
    const scenarioData = await backendService.fetchScenarioData(scenario.id);
  ...
  fetchScenario: async (scenarioId) => {
    try {
      const data = await backendService.fetchScenarioData(scenarioId);
      set({
        nodes: data.nodes || [],
        edges: data.edges || [],
        selectedNodeId: null,
  ...
  saveScenario: async (scenario) => {
    try {
      ...
      await backendService.saveScenarioData({
        scenario,
