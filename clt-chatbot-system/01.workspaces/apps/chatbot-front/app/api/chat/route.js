...
const actionHandlers = {
    'GET_SCENARIO_LIST': async (payload, slots, language, authHeader) => {
        try {
            const scenarios = await getScenarioList(authHeader);
    ...
    'START_SCENARIO': async (payload, slots, language, authHeader) => {
        const { scenarioId } = payload;
        try {
            const scenario = await getScenario(scenarioId, authHeader);
...
async function determineAction(messageText, language = 'ko', authHeader) {
    if (Object.keys(actionHandlers).includes(messageText)) {
        return { type: messageText };
    }
    const triggeredAction = await findActionByTrigger(messageText, authHeader);
    if (triggeredAction) {
    ...
    try {
        ...
        await getScenario(messageText, authHeader);
...
export async function POST(request) {
  ...
  try {
    ...
    if (scenarioSessionId && scenarioState && scenarioState.scenarioId) {
      const scenario = await getScenario(scenarioState.scenarioId, authHeader);
    ...
    if (scenarioSessionId && !scenarioState && message && message.text) {
        ...
        if (handler) {
            ...
            return await handler(payload, slots || {}, language, authHeader);
        } else {
    ...
    if (!scenarioState && message && message.text) {
        const action = await determineAction(message.text, language, authHeader);
        const handler = actionHandlers[action.type];

        if (handler) {
            return await handler(action.payload, slots, language, authHeader);
        }

        if (action.type === 'LLM_FALLBACK') {
            const categories = await getScenarioCategories(authHeader);
