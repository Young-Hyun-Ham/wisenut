...
const buildApiUrl = (baseUrl, params, slots) => {
    ...
    if (currentUrl.startsWith('/')) {
        const appBaseUrl = process.env.API_BASE_URL || (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'http://localhost:3000');
...
async function handleApiNode(node, scenario, slots) {
    ...
    const executeSingleApi = async (apiConfig) => {
        ...
        const response = await apiFetch(targetUrl, options);
...
async function handleLlmNode(node, scenario, slots, language) {
    ...
    if (!apiUrl) {
        ...
        return {
            ...
            events: [{
                type: "toast",
                message: "Flowise API URL이 설정되지 않았습니다.",
                toastType: "error",
            }],
        };
    }
    ...
    try {
        const response = await apiFetch(apiUrl, {
            ...
            body: JSON.stringify({ question: interpolatedPrompt, streaming: false }),
        });
        if (!response.ok) {
