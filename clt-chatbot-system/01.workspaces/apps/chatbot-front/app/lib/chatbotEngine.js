 ...
 */
export async function getScenarioCategories(authHeader) {
  const now = Date.now();
  ...
  try {
    const response = await apiFetch(`${BASE_URL}/shortcuts`, {
      headers: authHeader ? { Authorization: authHeader } : undefined,
    });
    if (!response.ok) throw new Error("Failed to load shortcuts.");
    const scenarios = await response.json();
    cachedScenarioCategories = scenarios;
    lastFetchTime = now;
    return cachedScenarioCategories;
  } catch (error) {
...
export async function findActionByTrigger(message, authHeader) {
  const scenarioCategories = await getScenarioCategories(authHeader);
...
export const getScenarioList = async (authHeader) => {
  const response = await apiFetch(`${BASE_URL}/scenarios`, {
    headers: authHeader ? { Authorization: authHeader } : undefined,
  });
...
export const getScenario = async (scenarioId, authHeader) => {
  // scenarioId 유효성 검사 추가
  if (!scenarioId || typeof scenarioId !== 'string') {
      throw new Error(`Invalid scenario ID provided: ${scenarioId}`);
  }
  const response = await apiFetch(`${BASE_URL}/scenarios/${scenarioId}`, {
    headers: authHeader ? { Authorization: authHeader } : undefined,
  });
  if (response.ok) {
