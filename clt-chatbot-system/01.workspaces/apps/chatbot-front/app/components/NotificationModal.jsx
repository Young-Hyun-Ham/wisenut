      ...
      {toastHistory.length === 0 ? (
        ...
        <ul className={styles.notificationList}>
          {toastHistory.map((toast) => (
            <li
              key={toast.id}
              className={`${styles.notificationItem} ${styles[toast.type]} ${
                !toast.read ? styles.unreadItem : ""
              }`}
              onClick={() => handleNotificationNavigation(toast)} // --- 👈 [수정]
            >
              <button
                className={styles.deleteButton}
                onClick={(e) => handleDelete(e, toast.id)}
              >
                <TrashIcon />
              </button>
              <span className={styles.timestamp}>
                {toast.created_at
                  ? new Date(toast.created_at).toLocaleString(language)
