...
* **하이브리드 채팅 시스템**: 시나리오 기반의 정형화된 응답과 외부 LLM(Flowise)을 통한 유연한 자연어 응답을 결합하여 제공합니다.
* **실시간/비동기 대화**: 백엔드 REST/SSE를 통해 메시지와 상태를 동기화합니다.
* **사용자 인증**: `dev-login` 기반 토큰 인증을 사용합니다.
...
* **Generative AI**: [Flowise](https://flowiseai.com/)
...
│   ├── lib/               # Core logic (LLM, Chatbot Engine)
...
* **`app/lib`**: LLM API 호출(`llm.js`), 시나리오 엔진(`chatbotEngine.js`, `nodeHandlers.js`) 등 핵심 비즈니스 로직을 포함합니다.
...
프로젝트의 핵심 기능 중 하나는 `app/lib/chatbotEngine.js`에 구현된 시나리오 엔진입니다. 이 엔진은 백엔드 API에서 제공하는 시나리오 정의를 기반으로 사용자와의 상호작용 흐름을 제어합니다.
...
1.  **시나리오 트리거**: `findActionByTrigger` 함수가 사용자 입력과 시나리오 목록을 비교하여 연결된 시나리오 또는 커스텀 액션을 찾습니다 (`app/lib/chatbotEngine.js`).
2.  **시나리오 로드**: `getScenario` 함수가 백엔드 API에서 해당 시나리오의 노드/엣지 데이터를 가져옵니다.
...
5.  **상태 저장**: 시나리오 진행 상태(현재 노드 ID, 슬롯 값 등)는 백엔드 API로 저장됩니다.

### 새로운 시나리오 추가하기

1.  **시나리오 에디터 사용**: (가이드) 시나리오를 시각적으로 설계할 수 있는 외부 툴(예: React Flow 기반의 자체 에디터)을 사용하여 노드와 엣지를 구성합니다.
2.  **백엔드 데이터 저장**: 설계된 시나리오의 JSON 데이터(`nodes`, `edges`, `version` 등 포함)를 백엔드 API에 저장합니다. 저장된 ID가 `scenarioId`가 됩니다.
3.  **숏컷 목록에 등록 (선택 사항)**: 사용자가 입력창 좌측 메뉴를 통해 시나리오를 직접 시작하게 하려면, `/admin/scenario-editor` 페이지에서 목록 구조를 편집하여 해당 시나리오를 추가합니다. 여기서 `action` 타입을 `scenario`로, `value`를 `scenarioId`로 설정합니다.
...
FLOWISE_API_URL=your_flowise_endpoint_url
...
Note: FLOWISE_API_URL은 선택 사항입니다. 이 값을 설정하지 않으면 관리자 페이지(/admin/general)에서 설정해야 합니다. 관리자 설정이 우선 적용됩니다.

Mock 모드 실행 방법:
- `.env.local`에 `APP_MODE=mock` 설정
- API 호출 대상은 `API_BASE_URL`로 지정 (예: `http://localhost:4010`)
...
* **New Scenario Addition**: 트리거 등록은 관리자 페이지(`/admin/scenario-editor`)에서 수행합니다.
* **Prerequisites**: Flowise 관련 요구사항 추가.
* **Environment Variables**: `FLOWISE_API_URL` 추가 및 설명. `PARENT_ORIGIN` 추가.
